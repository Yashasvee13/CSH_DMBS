...
SET datestyle = 'DMY';
ALTER DATABASE "Assignment2_Testing" SET datestyle = 'ISO, DMY';
...
CREATE OR REPLACE FUNCTION check_admin_login_procedure(
    login VARCHAR(10), 
...
BEGIN
    ...
    FROM 
        Administrator
    WHERE 
        LOWER(Administrator.UserName) = LOWER(login)
        AND Administrator.Password = p_password;
END;
$$ LANGUAGE plpgsql;



CREATE OR REPLACE FUNCTION find_admissions_by_admin_procedure(
...
BEGIN
    ...
    SELECT 
        ...
        d.DeptName, 
        TO_CHAR(a.DischargeDate, 'DD-MM-YYYY')::VARCHAR(10) AS discharge_date, 
        a.Fee, 
        (p.FirstName || ' ' || p.LastName)::VARCHAR(100) AS patient_name, 
        a.Condition
    FROM 
        Admission a
    JOIN 
